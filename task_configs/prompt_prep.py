...
def route_prompt_formatting(
    ...
    """
    Routes to the appropriate prompt formatting function based on the task_type.

    Args:
        task_type (str): The type of task, e.g., "generate_employee_goals" or "llm_judge_evaluate_goal".
        employee_context (dict): Dictionary containing employee information.
        llm_input_args_config (dict): Configuration for LLM input arguments.
        goal (str, optional): The goal to be evaluated. Required for "goal_evaluation".
                               Defaults to None.
    ...
    Raises:
        ValueError: If an unsupported task_type is provided or if 'goal' is missing
                    for 'llm_judge_evaluate_goal'.
    """
    if task_type == "generate_employee_goals":
        return format_goal_generation_prompt(employee_context, llm_input_args_config)
    elif task_type == "llm_judge_evaluate_goal":
        if goal is None:
            raise ValueError(
                "The 'goal' argument is required for 'llm_judge_evaluate_goal' task type."
            )
        return format_llm_judge_evaluate_goal_prompt(
            goal, employee_context, llm_input_args_config
        )
    else:
